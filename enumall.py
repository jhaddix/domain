...
# enumall is a refactor of enumall.sh providing a script to identify subdomains
# using several techniques and tools.  Relying heavily on the stellar Recon-NG
# framework and Alt-DNS, enumall will identify subdomains via search engine
# scraping (yahoo, google, bing, baidu), identify subdomains using common OSINT
# sites (shodan, netcraft), identify concatenated subdomains (altDNS), and
# brute-forces with a stellar subdomain list (formed from Bitquark's subdomain
...
parser.add_argument("domains", nargs="*", default=None,
                    help="one or more domains", )
parser.add_argument("-w", dest="wordlist", type=argparse.FileType('r'),
                    default=None,
                    help="input file of subdomain wordlist. must be in same "
                         "directory as this file, or give full path")
parser.add_argument("-p", dest="permlist", type=argparse.FileType('r'),
                    default=None,
                    help="input file of permutations for alt-dns. if none "
                         "specified will use default list.")
...
if args.runAltDns:
    ...
    permList = args.permlist.name if args.permlist else os.path.join(altDnsPath,
                                                                     "words.txt")
    output = os.path.join(os.getcwd(), workspace + "_output.txt")
    print("running alt-dns... please be patient :) "
    ...
    os.system('%s -i %s -o data_output -w %s -r -s %s' % (altCmd, subdomains,
                                                          permList, output))
